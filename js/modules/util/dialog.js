define(function(require){
    var $ =require("jquery");
    var dialogOnly = null;
    var initDialog = function(settings) {
        if(dialogOnly){
            return;
        }
        dialogOnly = new Dialog();
        if (settings.type === "alert") {
            ...
            if (settings.button) {
                if (typeof settings.button == 'boolean') {
                    settings.button = settings.buttonText;
                };
            ...
            if(settings.clickMask === true){
                dialogOnly.touch(dialogOnly.mask, function() {
                    dialogOnly.hide();
                    settings.callback && settings.callback();
                });
            }
        ...
        if (settings.type === "confirm") {
            ...
            var options = $.extend({
                ...
                beforeShow: function(c) {
                    dialogOnly.touch($('.ui-confirm-submit', c), function() {
                        settings.callback && settings.callback.call(dialogOnly, 'yes', c);
                    });
                    dialogOnly.touch($('.ui-confirm-no', c), function() {
                        settings.callback && settings.callback.call(dialogOnly, 'no', c);
                    });
                    dialogOnly.touch($('.ui-confirm-close', c), function() {
                        settings.callback && settings.callback.call(dialogOnly, 'close', c);
                    });
                }
            }, settings);
            dialogOnly.init(options);
        }
    };
    /*alert*/
    var alertDialog = function(content, button, callback, timer, settings) {
        var options = {};
        var defaults = {
            zIndex: 9999,
            type: 'alert'
        ...
        initDialog($.extend(options, settings));
    }
    /*
     buttons :[{yes:"确定"},{no:'取消'},{close:'关闭'}]
     */
    var confirmDialog = function(content, buttons, callback, settings) {
        var options = {};
        var defaults = {
            zIndex: 9999,
            type: 'confirm'
    ...
    Dialog.prototype = {
        init: function(settings) {
            ...
            var zIndex = this.settings.zIndex || 9999;
